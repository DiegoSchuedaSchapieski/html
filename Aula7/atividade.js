...
ativos = users.filter((p) => p.active == true)

desativados = users.filter((p) => p.active != true)

manager = users.filter((p) => p.role == "manager")

designer = users.filter((p) => p.role == "designer")

analist = users.filter((p) => p.role == "data_analyst")

sa5000 = users.filter((p) => p.salary > 5000)

sa4000 = users.filter((p) => p.salary <= 4000)

age30 = users.filter((p) => p.age < 30)

age20 = users.filter((p) => p.age > 20)

dev4000 = users.filter((p) => p.role == "dev" & p.salary > 4000)

man30 = users.filter((p) => p.role == "manager" & p.age > 30)
