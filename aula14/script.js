...
const agrupado = vendas.reduce((acc, venda) => {
    const existente = acc.find(item => item.categoria == venda.categoria);
    if (existente) {
        existente.total += venda.quantidade;
    } else {
        acc.push({
...
const agrupado_cidade = vendas.reduce((c, venda) => {
    const faturamentoAtual = venda.preco * venda.quantidade; 

    const existente = c.find(item => item.cidade == venda.cidade);
    if (existente) {
        existente.total += faturamentoAtual;
    } else {
        c.push({
            cidade : venda.cidade,
...
new Chart(ctx3,{
    type:'bar',
    data: {
        labels: agrupado_cidade.map((p) =>{
            return p.cidade
        }),
        datasets: [{
            label: 'Cidades;',
            data:agrupado_cidade.map((p) => {
                return p.total
            }),
            borderWidth: 1
        }]    
